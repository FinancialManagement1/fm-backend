...
namespace BusinessLogic.Mappers
{
    public class TransactionMapper
    {
        ...
        public static Transaction ToModel(CreateTransactionRequest transaction) 
        { 
            return new Transaction
            {
                Amount = transaction.Amount,
                Type = Enum.Parse<TransactionType>(transaction.Type, true),
                Currency = transaction.Currency ?? "USD",
                Category = transaction.Category,
                Description = transaction.Description,
                Date = DateTime.ParseExact(transaction.Date, "yyyy-MM-dd", CultureInfo.InvariantCulture)
            };
        }
    }
}